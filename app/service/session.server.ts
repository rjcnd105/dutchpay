...
const getSessionFromHeaders = ({ headers }: Pick<Request, 'headers'>) =>
  getSession(headers.get('Cookie'));

type WithSessionFn<R> = (
  args: DataFunctionArgs & { session: Session },
) => Promise<R>;
...
const flushPageSession =
  (session: Session) => (key: keyof typeof PAGE_SESSION_KEYS, value: Message) =>
    session.flash(key, value);

function getPageSessionData(
  session: Session,
  key: keyof typeof PAGE_SESSION_KEYS,
) {
  return O.fromNullable<Message>(session.get(key));
}

const unsetSessionData =
  (session: Session) => (key: keyof typeof PAGE_SESSION_KEYS) =>
    session.unset(key);
