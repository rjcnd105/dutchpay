...
export const loader = async (args: DataFunctionArgs) => {
  ...
  return null;
  // return db.exceptedPayItemsForPayers.({
  //   where: {
  //     payerId_payItemId: {
  //       payItemId,
  //       payerId,
...
export default function calculate() {
  ...
  const payItemIds = useMemo(
    () => room.payItems.map(payItem => payItem.id),
    [room.payItems],
  );
        ...
        <div className="flex-auto overflow-y-auto">
          {room.payItems.map((payItem, i) => {
            const amountPerPayer =
              payItem.exceptedPayers.length === 0
                ? payItem.amount
                : Math.floor(payItem.amount / payItem.exceptedPayers.length);
            const exceptedPayerSet = new Set(
              payItem.exceptedPayers.map(({ payerId }) => payerId),
            );
                ...
                {room.payers.map(payer => {
                  ...
                  return (
                    <li
                      key={key}
                      className={clsx(
                        'flex min-w-[70px] h-full border-b-primary200 border-b-1',
                        bg,
                      )}>
                      {/*<Checkbox*/}
                      {/*  id={id}*/}
                      ...
                      {/*  <span*/}
                      {/*    key={key}*/}
                      {/*    className="text-darkgrey100 text-caption2">*/}
